...
fn handle_add_item(list_item: ListItem, list_addr: HashString) -> serde_json::Value {
	match hdk::commit_entry("listItem", json!(list_item))
		.and_then(|item_addr| {
			...
			match hdk::get_links(&list_addr, "items") { // if successful, try to load the linked items
				Ok(result) => {
					let items: Vec<ListItem> = result.links.iter()
						...
						.collect(); // collect all the Ok items in to a list
...
define_zome! {
    ...
    functions: {
        // "main" is the name of the group
        // "Public" is the access setting of the group
        main (Public) {
            create_list: {
                inputs: |list: List|,
